...
| `std.Window` | Wayland desktop window (instance): `open(w, h, title)` (`null` without a compositor; creates an `xdg_toplevel` so the compositor maps it), `close()` (tears the connection down so the compositor removes the window — answer `CLOSE` with it, or the window stays mapped and goes "not responding"), `minimize()` (taskbar; the user restores it), `running()`, `width()`, `height()`, `present(Image)`, `nextEvent()`, `peekEvent()`, `evType()`/`evX()`/`evY()`/`evButton()`/`evKey()`/`evState()`/`evMods()`, static event codes `FRAME`/`KEY`/`BUTTON`/`MOTION`/`CLOSE`/`CONFIGURE`, helpers `fixed(v)` (24.8), `keyChar(code)` (Linux evdev keycodes; 8/9/13/27 for Backspace/Tab/Return/Escape), `parseDisplay(name)`, `xdgPath(dir, name)`, `strEq(a, b)`; answers compositor ping (never "not responding"), each `Window` is its own connection (several can be open at once; pump + present each), `CLOSE` is reported at most once (further peeks return 0), requests server-side decorations via `xdg-decoration` when offered (KWin title bar with min/max/close), follows resizes (rebuilds buffers; `width()`/`height()` update), `present` never blocks (hidden windows keep running) |
| `std.App` | the normal path for a windowed app (file `gui.flint`): `open(w, h, title)` (static; `null` without a compositor), `run()` (pump + dispatch + present; 0 on return, no-op without a window), `stop()`, `window()`, `canvas()`, `running()`, `noEscapeQuit()`; handler fields `setup`/`draw`/`key`/`button`/`motion`/`resize`/`close` registered with `+=` (C#-style) implementing `OnSetup`/`OnDraw`/`OnKey`/`OnButton`/`OnMotion`/`OnResize`/`OnClose`. Escape quits by default; the close button and a dead connection stop the loop and call `OnClose`. The raw `std.Window` API stays for advanced use (e.g. pumping several windows at once) |
| `std.Widget` | widgets for `std.Window` (file `widget.flint`): `Widget` (`hit(px, py)`), `Label` (`text()`, `setText(t)`, `draw(im)`), `TextBox` (single-line input: `text()`, `length()`, `entered()`, `onKey(code, mods)`, `clear()`, `draw(im)`; focus + shift-aware US key map), `Gui` (`open(w, h, title)` (static; `null` without a compositor), `run()` (pump + dispatch + present; 0 on return, no-op without a window), `stop()`, `window()`, `running()`, `setRunning(v)`, `noEscapeQuit()`, `step()` (pump + dispatch; returns the last event code), `drawWidgets(im)`, `addLabel`/`addBox`/`addButton`/`addCheck`/`addRadio`/`addSlider`/`addDrop`/`addPass`/`addNum`/`addArea` (capacity 8 each), `nLabels()`, `nBoxes()` (and one counter per kind); handler fields `setup`/`draw`/`key`/`button`/`motion`/`resize`/`close` registered with `+=`) |
...
2. **`std.*` modules**: a standard library written entirely in Flint
   ...
   `std.Str`, `std.Time`, `std.Image`, `std.Thread`, `std.Socket`,
    `std.Sync`, `std.Mem`, `std.Window`, `std.App`, `std.Widget`. **Prefer
